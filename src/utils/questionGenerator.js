import { KONFIGURASI_KELAS } from './levelConfig';
import { ambilKonfigurasiAdmin } from './adminConfig';
import { EnumOperasi } from '../models/Enums';
...
const genPenjumlahan = (batasMaksimal) => {
  ...
  return { soal: `${angka1} + ${angka2}`, jawaban: (angka1 + angka2).toString(), jenis: EnumOperasi.PENJUMLAHAN };
};
...
const genPengurangan = (batasMaksimal) => {
  ...
  return { soal: `${a} - ${b}`, jawaban: (a - b).toString(), jenis: EnumOperasi.PENGURANGAN };
};
...
const genPerkalian = (tingkatKelas) => {
  ...
  return { soal: `${angka1} × ${angka2}`, jawaban: (angka1 * angka2).toString(), jenis: EnumOperasi.PERKALIAN };
};
...
const genPembagian = (tingkatKelas) => {
  ...
  return { soal: `${angkaYangDibagi} ÷ ${pembagi}`, jawaban: hasilBagi.toString(), jenis: EnumOperasi.PEMBAGIAN };
};
...
const genPositifNegatif = (batasMaksimal) => {
  ...
  return { soal: `${angka1} ${operator} ${strAngka2}`, jawaban: jawabanAkhir.toString(), jenis: EnumOperasi.POSITIF_NEGATIF };
}
...
const genCampuran = () => {
  ...
  if (ambilAngkaAcakInt(0, 1) === 0) {
    return { soal: `${A} + ${B} × ${C}`, jawaban: (A + B * C).toString(), jenis: EnumOperasi.CAMPURAN };
  } else {
    return { soal: `${B} × ${C} - ${A}`, jawaban: (B * C - A).toString(), jenis: EnumOperasi.CAMPURAN };
  }
}
...
const genDesimal = () => {
  ...
  return { soal: `${a} ${operator} ${b}`, jawaban: jawabanAkhir.toString(), jenis: EnumOperasi.DESIMAL };
}
...
const genBangunDatar = (batasMaksimal) => {
  ...
  return { soal: teksSoal, jawaban: jawabanAkhir.toString(), jenis: EnumOperasi.BANGUN_DATAR };
};
...
export const generateQuestion = (tingkatKelas, materiAktif = []) => {
  ...
  const batasMaksimal = adminConfig.batasAngkaKelas[tingkatKelas] || 20;

  if (materiAktif.length === 0) materiAktif = config.materi;
  const topik = materiAktif[ambilAngkaAcakInt(0, materiAktif.length - 1)];

  switch (topik) {
    case EnumOperasi.PENJUMLAHAN: return genPenjumlahan(batasMaksimal);
    case EnumOperasi.PENGURANGAN: return genPengurangan(batasMaksimal);
    case EnumOperasi.PERKALIAN: return genPerkalian(tingkatKelas);
    case EnumOperasi.PEMBAGIAN: return genPembagian(tingkatKelas);
    case EnumOperasi.DESIMAL: return genDesimal();
    case EnumOperasi.PECAHAN: return genPecahan();
    case EnumOperasi.POSITIF_NEGATIF: return genPositifNegatif(batasMaksimal);
    case EnumOperasi.CAMPURAN: return genCampuran();
    case EnumOperasi.BANGUN_DATAR: return genBangunDatar(batasMaksimal);
    default: return genPenjumlahan(batasMaksimal);
  }
};

export const checkAnswer = (objekSoal, inputPengguna) => {
  if (inputPengguna === undefined || inputPengguna === null || inputPengguna === '') return false;
  const inputStr = String(inputPengguna).trim();
  if (!inputStr) return false;

  if (objekSoal.jenis === EnumOperasi.PECAHAN) {
    let inputAtas, inputBawah;
    if (inputStr.includes('/')) {
      const bagian = inputStr.split('/');
  ...
  return inputStr === objekSoal.jawaban.toString();
};
